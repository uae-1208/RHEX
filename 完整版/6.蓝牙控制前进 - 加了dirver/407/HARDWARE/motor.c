...
void MOTOR_Angle_Init(Motor_t* motor1,Motor_t* motor2,Motor_t* motor3,Motor_t* motor4,Motor_t* motor5,Motor_t* motor6)
{
	//机体收缩时的编码器值
	motor1->angle_accmul = 1503;
	motor2->angle_accmul = -1643;
	motor3->angle_accmul = -1662;
	motor4->angle_accmul = 1500 ;
	motor5->angle_accmul = -1653;
	motor6->angle_accmul = -1666;
	
	motor1->pid_angle.target = angle_To_encoder(30);
	motor3->pid_angle.target = angle_To_encoder(30);
	motor5->pid_angle.target = angle_To_encoder(30);

	motor2->pid_angle.target = angle_To_encoder(-10);
	motor4->pid_angle.target = angle_To_encoder(-10);
	motor6->pid_angle.target = angle_To_encoder(-10);
...
//速度环控制:速度环目标为正则输出为正;速度环输出正负决定PWM正负,PWM正负决定转动方向。
static void MOTOR_Speed_Loop(Motor_t* motor1,Motor_t* motor2,Motor_t* motor3)
{
	Pid_Calulate(&(motor1->pid_speed));
	Pid_Calulate(&(motor2->pid_speed));
	Pid_Calulate(&(motor3->pid_speed));
...
//角度环控制:总角度误差为正时输出给速度环的值为正
static void MOTOR_Angle_Loop(Motor_t* motor1,Motor_t* motor2,Motor_t* motor3)
{
	Pid_Calulate(&(motor1->pid_angle));			       //角度环计算
	Pid_Calulate(&(motor2->pid_angle));			      
	Pid_Calulate(&(motor3->pid_angle));			      
	...
	MOTOR_Speed_Loop(motor1,motor2,motor3);
}


void MOTOR_Speed_Task(Motor_t* motor1,Motor_t* motor2,Motor_t* motor3)
{
	MOTOR_UPDATE(motor1);                          //电机数据更新
	MOTOR_UPDATE(motor2);                           
	MOTOR_UPDATE(motor3);                         
...
void MOTOR_Angle_Task(Motor_t* motor1,Motor_t* motor2,Motor_t* motor3)
{
	MOTOR_UPDATE(motor1);                          //电机数据更新
	MOTOR_UPDATE(motor2);                           
	MOTOR_UPDATE(motor3);                         
	...
	MOTOR_Angle_Loop(motor1,motor2,motor3);
}