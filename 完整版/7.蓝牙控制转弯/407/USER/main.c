...
int main(void)
{
	...
	while(1)
    {
		if(Pid_cal_Task == TASK_ENABLE)
		{
			...
			{
				motor1.pid_speed.target = 20;
				motor3.pid_speed.target = 20;
				motor5.pid_speed.target = -20;
			...
			if(Blue_Task_flag == speed_pid_Task)              //蓝组尚未转到缓冲区,所以执行速度环
			{
				...
				motor2.pid_speed.target = -20;
				motor4.pid_speed.target = 20;
				motor6.pid_speed.target = 20;
            ...
            {    
				if(order == LEFT_TURN)
				{
					...
					motor2.pid_angle.target -= angle_To_encoder(360);		//1:40.5 2:40.5 3:40.5 5:38 6:40.5		
					motor4.pid_angle.target += angle_To_encoder(360);				
					motor6.pid_angle.target += angle_To_encoder(360);
				}							
			    else if(order == RIGHT_TURN)
				{
					Station_change_Task = 200;       //计时0.2s
					
					motor1.pid_angle.target += angle_To_encoder(360);		//1:40.5 2:40.5 3:40.5 5:38 6:40.5		
					motor3.pid_angle.target += angle_To_encoder(360);				
					motor5.pid_angle.target -= angle_To_encoder(360);
				}							
					white_flag = 0;
					blue_flag = 0;
			}
